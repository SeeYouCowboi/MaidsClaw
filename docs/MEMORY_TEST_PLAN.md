...
```text
请按 docs/MEMORY_TEST_PLAN.md 对 MaidsClaw 的记忆系统做最小但全面的验证，不要只跑单元测试。

必须同时完成两类结论：
1. Subsystem Validity：memory 子系统本身是否成立
2. Runtime Consumption：真实运行时是否真的在使用这些抽象层

请严格按 MEM-01 到 MEM-14 执行，每项输出：
- Status: PASS / WARN / FAIL
- Evidence
- Judgment
- Risk
...
重点回答这些问题：
- 图推理链是否真实有效
- 记忆整理 organize 是否真的发挥作用
- promotion 是否真的可靠且不会泄漏私有信息
- 两平面权威层知识是否在 retrieval 和 graph traversal 中都被严格执行
- PromptBuilder / core memory / memory hints 是否真正进入 live prompt
- memory tools 是否真的被 live runtime 注册
- turn -> interaction -> flush -> job -> MemoryTaskAgent 这条链今天是否真的接通

不要因为 src/memory 的测试通过就默认系统有效。如果某项能力只存在于测试或辅助模块、没有接到 AgentLoop / Gateway / 启动入口，请明确标记为 WARN 或 FAIL。
```
