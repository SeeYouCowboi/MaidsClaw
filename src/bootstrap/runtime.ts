...
import { type ErrorCode, MaidsClawError } from "../core/errors.js";
...
import { PgSearchLexicalBackend } from "../storage/domain-repos/pg/pg-search-backend.js";
...
class LightweightLlmServiceImpl implements LightweightLlmService {
	...
	async complete(
		options: LightweightCompleteOptions,
	): Promise<LightweightCompleteResult> {
		...
		if (!model) {
			...
			throw new MaidsClawError({
				...
				message:
					"No chat model is available for lightweight completion. Set runtime.json lightweightLlm.defaultChatModelId or configure an auto-default provider.",
				retriable: false,
			});
		}
		...
		try {
			...
			const code =
				error instanceof MaidsClawError ? error.code : "INTERNAL_ERROR";
			console.error(
...
export function bootstrapRuntime(
	...
	const talkerThinkerConfig: RuntimeBootstrapResult["talkerThinkerConfig"] = {
		...
		softBlockPollIntervalMs:
			talkerThinkerFromConfig?.softBlockPollIntervalMs ?? 500,
		...
		legacyAreaStateCompat:
			talkerThinkerFromConfig?.legacyAreaStateCompat ?? false,
	...
	const runtimeConfigSnapshot: RuntimeConfig = {
		...
		talkerThinker: {
			...
			entityCanonicalization: {
				...
				judgeModelId: talkerThinkerConfig.entityCanonicalization.judgeModelId,
				judgeEnabled: talkerThinkerConfig.entityCanonicalization.judgeEnabled,
				judgeBatchIntervalMs:
	...
	const pgSearchLexicalBackend = createLazyPgRepo(
		() => new PgSearchLexicalBackend(resolvePgPool()),
	);
	const pgNarrativeSearchRepo = createLazyPgRepo(
		() => new PgNarrativeSearchRepo(resolvePgPool(), pgSearchLexicalBackend),
	);
	...
	const retrievalOrchestrator = new RetrievalOrchestrator({
		...
		episodeSearchFn: async (query, agentId, limit) => {
			const hits = await pgSearchLexicalBackend.searchEpisode({
				query,
				agentId,
				limit,
			});
