...
export function getSettlementRecord(
  ...
  const unsafeRawMode = resolveUnsafeRawMode(params.mode, params.unsafeRaw ?? false);
  if (unsafeRawMode) {
    return settlement;
  }
...
export function getRequestEvidence(params: {
  ...
  const trace = getTraceBundle(params.runtime, params.requestId, params.traceStore);
  const interactionStore = getInteractionStore(params.runtime);
  const context = completeContext(params.context, params.requestId, interactionStore, trace);
  ...
  const settlementPayload = context.sessionId
...
function completeContext(
  ...
  const sessionId = resolveSessionId(context, requestId, interactionStore, trace);
  return {
...
function resolveSessionId(
  ...
  if (requestedSession && hasRequestInSession(interactionStore, requestedSession, requestId)) {
    return requestedSession;
  }

  const sessionFromRequest = interactionStore.findSessionIdByRequestId(requestId);
  if (sessionFromRequest) {
    return sessionFromRequest;
  }

  const sessionFromTrace = normalize(trace?.session_id);
  if (sessionFromTrace && hasRequestInSession(interactionStore, sessionFromTrace, requestId)) {
...
function resolveUnsafeRawMode(mode: InspectAccessMode | undefined, requested: boolean): boolean {
  if (!requested) {
    return false;
  }
