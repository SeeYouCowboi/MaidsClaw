...
export class TurnService {
	constructor(
		...
		private readonly talkerThinkerConfig: {
			enabled: boolean;
	...
	async *runUserTurn(params: RunUserTurnParams): AsyncGenerator<Chunk> {
		...
		if (this.settlementUnitOfWork && this.talkerThinkerConfig.enabled) {
			try {
				...
				if (ownerAgentId) {
					const versionGap = await this.settlementUnitOfWork.run(
						async (repos) =>
					...
					if (versionGap) {
						...
						conversationWindowSize = Math.max(
							MIN_WINDOW_MESSAGES,
							(gap + OVERLAP_TURNS) * 2,
						);
					}
				...
				/* fall through to full history */
			}
		}

		if (this.settlementUnitOfWork) {
			const pgRecords = await this.settlementUnitOfWork.run(async (repos) =>
				repos.interactionRepo.getMessageRecords(params.sessionId, {
	...
	private async *runRpTalkerTurn(
		...
		if (this.settlementUnitOfWork) {
			...
			if (gap > threshold * 2) {
				...
				while (Date.now() < deadline) {
					...
					const newGap = await this.settlementUnitOfWork.run(async (repos) =>
						repos.recentCognitionSlotRepo.getVersionGap(
							effectiveRequest.sessionId,
							ownerAgentIdForGap,
						),
		...
		let normalizedTurnInput: NormalizedTurnInput | undefined;
		if (this.talkerThinkerConfig.speakerNormalizationGate) {
			const latestUserText = getLatestUserMessage(effectiveRequest.messages);
			normalizedTurnInput = normalizeTurnInput(latestUserText);
			effectiveRequest.contextText = JSON.stringify(normalizedTurnInput);
		} else {
			effectiveRequest.contextText = undefined;
		}
		...
		for (let attempt = 1; attempt <= TALKER_MAX_ATTEMPTS; attempt += 1) {
			const attemptResult =
				await this.attemptRpTalkerBuffered(effectiveRequest);
			if (attemptResult.ok) {
		...
		if (!this.settlementUnitOfWork) {
			this.traceLog(
				requestId,
				"warn",
				"No PG settlement UoW — emitting RP turn without durable settlement",
			);
			if (canonicalOutcome.publicReply.length > 0) {
				const replyChunk: Chunk = {
					type: "text_delta",
		...
		try {
			...
			if (this.talkerThinkerConfig.speakerNormalizationGate) {
				if (normalizedTurnInput?.speechActs.includes("correction")) {
					correctionSuspected = true;
				}
			} else {
				const latestUserText = getLatestUserMessage(effectiveRequest.messages);
				if (latestUserText) {
					const trimmed = latestUserText
						.replace(/^[\s\p{P}]+/u, "")
						.toLowerCase();
					if (
						["actually", "wait", "其实", "等等", "算了"].some((p) =>
			...
			try {
				await this.settlementUnitOfWork.run(async (repos) => {
					await repos.settlementLedger.markTalkerCommitted(
						settlementId,
						ownerAgentId,
					);
				});
	...
	> {
		...
		try {
			canonicalOutcome = normalizeRpTurnOutcome(
				...
				{
					legacyAreaStateCompat: this.talkerThinkerConfig.legacyAreaStateCompat,
				},
			);
		} catch (error: unknown) {
			return {
					...
					},
					{
						...
						artifactEnforcementContext: {
							...
							ownerAgentId:
								...
								(await this.resolveQueueOwnerAgentId(
									effectiveRequest.sessionId,
								)),
	...
	private async resolveQueueOwnerAgentId(
		sessionId: string,
	...
	private async bootstrapRpCoreMemory(sessionId: string): Promise<void> {
		if (!this.coreMemoryService || !this.personaService) {
			return;
		}
		...
		const personaId = agentId.startsWith("rp:") ? agentId.slice(3) : agentId;
		...
		try {
			await this.coreMemoryService.initializeFromPersonaSnapshot(agentId, {
				content: card.persona,
				snapshot_source: "persona_config",
				snapshot_source_id: card.id,
				snapshot_captured_at: Date.now(),
			});
		} catch {
			// Idempotent — block may already be populated or blocks not yet initialized
		}
	}
	...
	private async recordMaidenDecision(
		...
		const targetAgentId = await this.resolveQueueOwnerAgentId(
			request.sessionId,
		);
		const action =
