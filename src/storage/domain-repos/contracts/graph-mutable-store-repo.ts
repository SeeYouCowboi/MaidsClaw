...
export type WorldStateFactEdgeParams = {
...
export interface GraphMutableStoreRepo {
	createProjectedEvent(params: {
		...
		origin: Extract<
			EventOrigin,
			"runtime_projection" | "delayed_materialization"
		>;
	...
	createWorldStateFactEdge(
		params: WorldStateFactEdgeParams,
	): Promise<{ id: number; created: boolean }>;
	activeFactEdgesByOwner(
		sourceEntityId: number,
		predicate: string,
		targetEntityId: number,
		ownerAgentId: string,
	...
	updatePrivateEventLink(
		privateEventId: number,
		publicEventId: number,
	): Promise<void>;
	createSameEpisodeEdges(
		events: Array<{
			id: number;
			session_id: string;
