...
import type { AppendResult, CoreMemoryBlock, CoreMemoryLabel, ReplaceResult } from "./types.js";
...
export class CoreMemoryService {
  ...
  async getBlock(agentId: string, label: CoreMemoryLabel): Promise<CoreMemoryBlock & { chars_current: number; chars_limit: number }> {
  ...
  async replaceBlock(agentId: string, label: CoreMemoryLabel, oldText: string, newText: string, callerRole?: string): Promise<ReplaceResult> {
