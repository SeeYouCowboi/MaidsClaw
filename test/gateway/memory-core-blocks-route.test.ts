...
describe("memory core-block routes", () => {
  ...
  it("returns empty list when no core blocks exist", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/v1/agents/agent-empty/memory/core-blocks`,
    );
    expect(res.status).toBe(200);
  ...
  it("returns one core block detail", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/v1/agents/agent-a/memory/core-blocks/pinned_summary`,
    );
    expect(res.status).toBe(200);
  ...
  it("returns 404 NOT_FOUND when requested core block does not exist", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/v1/agents/agent-a/memory/core-blocks/persona`,
    );
  ...
  it("returns pinned summaries from current visible core blocks", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/v1/agents/agent-z/memory/pinned-summaries`,
    );
    expect(res.status).toBe(200);
  ...
  it("returns snapshot provenance when persona block has been bootstrapped", async () => {
    const blocks: CoreBlock[] = [
      {
        id: 1,
        agent_id: "rp:alice",
        label: "persona",
        description: null,
        value: "Alice是一位温柔、体贴且细心的专业女仆。",
        char_limit: 4000,
        read_only: 0,
        updated_at: 1700000000500,
        chars_current: 18,
        snapshot_source: "persona_config",
        snapshot_source_id: "alice",
        snapshot_captured_at: 1700000000400,
      },
    ];
    ...
    startServer({
      coreMemory: {
        ...
        replaceBlock: async () => ({ success: true, chars_current: 0 }),
        initializeFromPersonaSnapshot: async () => true,
      },
    });

    const res = await fetch(`${baseUrl}/v1/agents/rp:alice/memory/core-blocks`);
    expect(res.status).toBe(200);
    ...
    expect(body.blocks[0].label).toBe("persona");
    expect(body.blocks[0].content).toBe(
      "Alice是一位温柔、体贴且细心的专业女仆。",
    );
    expect(body.blocks[0].snapshot_source).toBe("persona_config");
    expect(body.blocks[0].snapshot_source_id).toBe("alice");
    expect(body.blocks[0].snapshot_captured_at).toBe(1700000000400);
  });

  it("omits snapshot provenance fields when persona block has no snapshot", async () => {
    const blocks: CoreBlock[] = [
      {
        id: 1,
        agent_id: "rp:alice",
        label: "persona",
        description: null,
        value: "manually written persona",
        char_limit: 4000,
        read_only: 0,
        updated_at: 1700000000500,
        chars_current: 24,
    ...
    expect(body.blocks).toHaveLength(1);
    expect(body.blocks[0]).not.toHaveProperty("snapshot_source");
    expect(body.blocks[0]).not.toHaveProperty("snapshot_source_id");
    expect(body.blocks[0]).not.toHaveProperty("snapshot_captured_at");
  });
});
