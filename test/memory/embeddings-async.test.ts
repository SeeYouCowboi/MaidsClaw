...
import { CONSENSUS_EDGE_SEMANTICS_MATRIX } from "../../src/memory/contracts/relation-contract.js";
import type { CoreMemoryService } from "../../src/memory/core-memory.js";
import {
	EmbeddingLinker,
	type OrganizerEmbeddingEntry,
	type OrganizerNode,
} from "../../src/memory/embedding-linker.js";
...
} from "../../src/storage/domain-repos/contracts/node-scoring-query-repo.js";
...
class CapturingEmbeddingRepo implements EmbeddingRepo {
	...
	> {
		...
		return [
			{ nodeRef: "event:99" as NodeRef, similarity: 0.91, nodeKind: "event" },
		];
	}

	async cosineSearch(): Promise<
		Array<{ nodeRef: NodeRef; similarity: number; nodeKind: string }>
	> {
		return [];
	}
...
class StubNodeScoringQueryRepo implements NodeScoringQueryRepo {
	constructor(
		...
		private readonly latestEmbedding: Float32Array | null = new Float32Array([
			0.7,
		]),
	) {}

	async getNodeRenderingPayload(
		nodeRef: NodeRef,
	): Promise<NodeRenderingPayload | null> {
...
describe("EmbeddingService async migration", () => {
	it("queryNearestNeighbors returns Promise and forwards modelId", async () => {
		...
		const resultPromise = service.queryNearestNeighbors(
	...
	it("retrieval localizeSeedsHybrid awaits async neighbors and passes modelId", async () => {
		...
		expect(
			seeds.some((candidate) => String(candidate.node_ref) === "event:3"),
		).toBe(true);
	});
	...
	it("EmbeddingLinker.link semantic relation caps align with relation contract cardinalities", async () => {
		const semanticEdges: Array<{
			source: NodeRef;
			target: NodeRef;
			relation: string;
		...
		const linker = new EmbeddingLinker(
			...
			} as unknown as GraphStorageService,
			{
				queryNearestNeighbors: async (
					...
					options: { nodeKind?: string },
				) => {
					if (options.nodeKind === "event") {
						return [
							{
								nodeRef: "event:2" as NodeRef,
								similarity: 0.99,
							...
							{
								nodeRef: "event:4" as NodeRef,
								similarity: 0.97,
								nodeKind: "event",
							},
							{
								nodeRef: "event:5" as NodeRef,
								similarity: 0.96,
								nodeKind: "event",
							},
							{
								nodeRef: "event:6" as NodeRef,
								similarity: 0.95,
								nodeKind: "event",
							},
							{
								nodeRef: "event:7" as NodeRef,
								similarity: 0.94,
								nodeKind: "event",
							},
						];
			...
			async () => "content",
			async (_sourceRef, _sourceKind, _sourceContent, targetRef) => {
				...
				if (target.endsWith(":6") || target.endsWith(":7")) {
		...
		const nodes: OrganizerNode[] = [
			{ nodeRef: "event:1" as NodeRef, nodeKind: "event", content: "source" },
		];

		await linker.link(entries, nodes, "agent-1", "effective-organizer-model");

		const similar = semanticEdges.filter(
			(edge) => edge.relation === "semantic_similar",
		);
		const conflict = semanticEdges.filter(
			(edge) => edge.relation === "conflict_or_update",
		);
		const bridge = semanticEdges.filter(
			(edge) => edge.relation === "entity_bridge",
		);

		expect(similar).toHaveLength(
			CONSENSUS_EDGE_SEMANTICS_MATRIX.semantic_similar.cardinality_per_source ??
				0,
		);
		expect(conflict).toHaveLength(
			CONSENSUS_EDGE_SEMANTICS_MATRIX.conflict_or_update
				.cardinality_per_source ?? 0,
		);
		expect(bridge).toHaveLength(
			CONSENSUS_EDGE_SEMANTICS_MATRIX.entity_bridge.cardinality_per_source ?? 0,
		);
	});
	...
	it("GraphOrganizer awaits batchStoreEmbeddings and awaits isMutualTopFive neighbor query", async () => {
		...
		const organizer = new GraphOrganizer(
			new StubNodeScoringQueryRepo(() => {
				if (!batchStoreResolved) {
					throw new Error(
						"registerGraphNodeShadows called before batchStoreEmbeddings resolved",
					);
				}
			}),
			{
		...
		const isMutual = await (
			organizer as unknown as {
				isMutualTopFive: (
					sourceRef: NodeRef,
					targetRef: NodeRef,
					nodeKind:
		...
		expect(
			nearestCalls.some((call) => call.modelId === "effective-organizer-model"),
		).toBe(true);
	});
});
